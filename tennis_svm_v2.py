...
def apply_normalization(df, normalization, print_status):
    ...
    df.drop(['location', 'surface', 'surfaceType', 'opp1_url', 'opp2_url', 'opp1_weight', 'opp1_height', 'opp2_weight', 'opp2_height', 'opp1_hand', 'opp2_hand',
            'opp1_set1', 'opp2_set1', 'opp1_set2', 'opp2_set2', 'opp1_set3', 'opp2_set3', 'opp1_set4', 'opp2_set4', 'opp1_set5', 'opp2_set5', 'match_link',
...
def predict_winner(normalized_p_df, norm, clf, accuracy):
    matches = np.array(normalized_p_df.drop(['opp1', 'opp2', 'winner', 'first_set_winner', 'date', 'title'], 1))
    opp1s = np.array(normalized_p_df['opp1'])
    opp2s = np.array(normalized_p_df['opp2'])
    dates = np.array(normalized_p_df['date'])
    titles = np.array(normalized_p_df['title'])
    ...
    for row in matches:
        ...
        if probability >= 80 and accuracy >= 75:
            winnerslist.append([dates[i], titles[i], opp1s[i], opp2s[i], eval('opp' + str(prediction[0]) +'s[i]'), probability, int(abs(confidence[0])*100)])
            winner_string = str(accuracy) + "% - " + titles[i][:5] + " - " + opp1s[i] + " vs " + opp2s[i] + " - " + str(prediction[0]) + " - " + str(probability) + '% Prob - ' + str(int(abs(confidence[0])*100)) + ' Dist'
            try:
...
def predict_set1_winner(normalized_p_df, norm, clf, accuracy):
    matches = np.array(normalized_p_df.drop(['opp1', 'opp2', 'winner', 'first_set_winner', 'date', 'title'], 1))
    opp1s = np.array(normalized_p_df['opp1'])
    opp2s = np.array(normalized_p_df['opp2'])
    titles = np.array(normalized_p_df['title'])
    ...
    for row in matches:
        ...
        if probability >= 75:
            print(accuracy + " - " + titles[i][:5] + " - " + opp1s[i] + " vs " + opp2s[i] + " - " + str(prediction) + " - " + str(probability) + '% prob - ' + str(int(abs(confidence[0])*100)) + ' Dist'
...
def find_player_matches(normalized_p_df, normalized_df, normalization_dict):
    winners_list = []
    for i in range(len(normalized_p_df.index)):
        opp1 = normalized_p_df['opp1'][i]
        opp2 = normalized_p_df['opp2'][i]
        refined_normalized_df = pd.concat([normalized_df[normalized_df.opp1 == opp1],
                                           normalized_df[normalized_df.opp1 == opp2],
                                            normalized_df[normalized_df.opp2 == opp1],
                                            normalized_df[normalized_df.opp2 == opp2]])

        if len(refined_normalized_df) >= 50:
            winner_clf, accuracy = train_winner(refined_normalized_df)
            winners_list.extend(predict_winner(normalized_p_df[normalized_p_df.index == i], normalization_dict, winner_clf, accuracy))
            set1_winner_clf, accuracy = train_set1_winner(refined_normalized_df)
            predict_set1_winner(normalized_p_df[normalized_p_df.index == i], normalization_dict, set1_winner_clf, accuracy)
    return(winners_list)

def process(normalized_df, normalization_dict):
    p_df = pd.read_csv('../tennis/tennis_data_prediction.csv', encoding = 'iso-8859-1')
    normalized_p_df = apply_normalization(p_df, normalization_dict, False)

    winners_list = find_player_matches(normalized_p_df, normalized_df, normalization_dict)
    return(winners_list)
